...
describe('testArray func', () => {
  test('Should replace one array for another', () => {
    ...
    expect(updateArray(testArr, 1, testReplace)).toEqual([testArr[0], testReplace, testArr[2]]);
  });
  ...
  test('Should place element in the end of the arr if passed hightValye key', () => {
    ...
    expect(updateArray(testArr, notExistentValue, testReplace)).toEqual([...testArr, testReplace]);
  });
});
